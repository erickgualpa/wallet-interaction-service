...
class SpringJdbcCoreWalletSearchRepositoryIT(
  ...
  @Autowired private val retrieveAccountBalance: RetrieveAccountBalance
) : AbstractIntegrationTest() {
  @Test
  fun `return null when wallet matching id not exists`() {
    val walletId = WalletId(randomUUID().toString())
    val testSubject = SpringJdbcCoreWalletSearchRepository(jdbcTemplate, retrieveAccountBalance)
